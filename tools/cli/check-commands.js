...
function discoverCommandRoots() {
  const roots = [];
  const main = resolve(process.cwd(), "src", "features", "commands");
  roots.push(main);

  // Scan extensions/*/src/features/commands — no plugin imports
  const modulesDir = resolve(process.cwd(), "extensions");
  try {
    const mods = readdirSync(modulesDir, { withFileTypes: true })
      .filter(d => d.isDirectory())
      .map(d => resolve(modulesDir, d.name, "src", "features", "commands"));
    roots.push(...mods);
